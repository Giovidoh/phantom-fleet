...
export default function App() {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        (async () => {
            const p = await commitmentOf(
                packBoard(gg.playerFleet.board),
                gg.playerSalt,
            );
            const b = await commitmentOf(
                packBoard(gg.botFleet.board),
                gg.botSalt,
            );
            if (!cancelled) {
                setReveal({
    ...
    function buildPreview(): { cells: number[]; valid: boolean } | null {
        ...
        if (g.phase !== "placement" || placingDone || sealed || hov == null)
            return null;
        ...
        return {
            cells,
            valid: canPlace(placeFleet.board, x, y, def.size, horiz),
        };
    }
    ...
    async function fireAt(idx: number) {
        ...
        try {
            ...
            if (result === 1) {
                g.hitsOnBot++;
                const ship = g.botFleet.ships.find((s) =>
                    s.cells.includes(idx),
                );
                if (ship && ++ship.hits === ship.size)
                    sunk = ship.name.toUpperCase();
            }
    ...
    async function botTurn() {
        ...
        try {
            ...
            if (result === 1) {
                g.hitsOnPlayer++;
                const ship = g.playerFleet.ships.find((s) =>
                    s.cells.includes(idx),
                );
                if (ship && ++ship.hits === ship.size)
                    sunk = ship.name.toUpperCase();
    ...
    const enemyShipsShown =
        g.phase === "gameover"
            ? g.botFleet.ships
            : g.botFleet.ships.filter((s) => s.hits >= s.size);
    ...
    return (
        <ScreenFrame pings={3}>
            ...
            {g.phase === "battle" && (
                <TurnBanner
                    turn={g.turn === "player" ? "player" : "enemy"}
                    turnNumber={g.entries.length}
                />
            )}
            {g.phase === "gameover" && (
                <TurnBanner
                ...
                <main className="relative z-10 mx-auto flex w-full max-w-[1720px] flex-wrap items-stretch gap-6 px-6 py-6">
                        ...
                        <BoardGrid
                            ...
                            status={
                                <>
                                    HULL INTEGRITY{" "}
                                    <b className="font-normal text-phos-300">
                                        {TOTAL_SHIP_CELLS - g.hitsOnPlayer}/
                                        {TOTAL_SHIP_CELLS}
                                    </b>
                                </>
                            }
                    ...
                    <div className="board-wrap">
                        <BoardGrid
                            ...
                            onHover={(idx) =>
                                setLockCoord(idx == null ? null : uiCoord(idx))
                            }
